...
import { PatreonMenu } from "./PatreonMenu.tsx";
...
const PopOver = memo(({ standalone = false, role }: PopOverProps) => {
  ...
  const createADashboard = useCallback(
    async (
      ...
      if (dashName) {
        if (RESERVED_DB_KEYS.includes(dashName)) {
      ...
      reader.onload = async (e) => {
        ...
        try {
          ...
          if (parsed?.type === "folder-export") {
            ...
            if (RESERVED_DB_KEYS.includes(dashboardName)) {
              uploadRef!.current!.setCustomValidity(
                "That name is reserved for something under the hood. Rename the file!",
              );
              uploadRef!.current!.reportValidity();
              event.target.value = "";
              return;
    ...
    <main className={styles["pop-over"]}>
      {selectedDashboard ? (
        ...
        <>
          ...
          <div className={styles["info-menus-container"]}>
            <InfoMenu />
            <PatreonMenu />
          </div>
